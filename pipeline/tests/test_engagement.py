...
def test_kill_cone_strikes_prey_when_facing():
    ...
    rock = Asset("b0", "heavy", "blue", 0.0, 0.0, vx=1.0, vy=0.0)
    sci = Asset("r0", "skirmisher", "red", 1.0, 0.0)
...
def test_kill_cone_misses_prey_off_heading():
    ...
    rock = Asset("b0", "heavy", "blue", 0.0, 0.0, vx=1.0, vy=0.0)
    sci = Asset("r0", "skirmisher", "red", 0.0, 1.0)   # straight up; heading is +x
...
def test_kill_cone_needs_a_heading():
    ...
    rock = Asset("b0", "heavy", "blue", 0.0, 0.0, vx=0.0, vy=0.0)
    sci = Asset("r0", "skirmisher", "red", 0.9, 0.0)
    assert resolve_engagements([rock, sci], t=0.0) == []
    assert sci.active == 1
...
def test_collision_destroys_both_on_body_overlap():
    ...
    a = Asset("b0", "heavy", "blue", 0.0, 0.0, vx=1.0, vy=0.0)
    b = Asset("r0", "heavy", "red", 0.5, 0.0, vx=-1.0, vy=0.0)
...
def test_collision_kills_allies_too():
    a = Asset("b0", "heavy", "blue", 0.0, 0.0)
    b = Asset("b1", "skirmisher", "blue", 0.5, 0.0)   # d 0.5 < 0.45+0.30
...
def test_allies_in_kill_cone_are_safe():
    ...
    rock = Asset("b0", "heavy", "blue", 0.0, 0.0, vx=1.0, vy=0.0)
    ally = Asset("b1", "skirmisher", "blue", 1.0, 0.0)   # d 1.0 > 0.75 (no collision)
...
def test_corpse_is_a_hazard_on_body_overlap():
    live = Asset("r0", "skirmisher", "red", 0.0, 0.0, vx=0.0, vy=1.0)
    corpse = Asset("b0", "heavy", "blue", 0.5, 0.0, active=0)   # d 0.5 < 0.3+0.45
...
def test_no_contact_no_effect():
    a = Asset("b0", "heavy", "blue", 0.0, 0.0, vx=1.0, vy=0.0)
    b = Asset("r0", "skirmisher", "red", 5.0, 5.0)
    assert resolve_engagements([a, b], t=0.0) == []
    assert survivors([a, b]) == {"blue": 1, "red": 1}


def test_greedy_leader_targets_nearest_prey():
    rock = Asset("b0", "heavy", "blue", 0.0, 0.0)
    near_sci = Asset("r0", "skirmisher", "red", 1.0, 0.0)
    far_sci = Asset("r1", "skirmisher", "red", 9.0, 0.0)
    paper = Asset("r2", "balanced", "red", 0.5, 0.0)
